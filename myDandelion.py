...
# per vedere dove siamo arrivati
    ...
    for t in tweets:
        ...
        all_tweets += text+'. ' #concateniamo il testo del tweet che stiamo analizzando
        index_tweets[id_tweet]= len(all_tweets) #settiamo l'indice di fine del tweet che stiamo analizzando
...
def binarySearch(index_tweets, item):
    ...
    found = False
    if item>index_tweets[last][1] or item<first:
        return -1
    while first<=last and not found:
        midpoint = (first + last)//2
        if index_tweets[midpoint][1] > item and index_tweets[midpoint-1][1]< item:
            found = midpoint
        else:
            if item < index_tweets[midpoint][1]:
                last = midpoint-1
            else:
                first = midpoint+1
...
#    capiamo quante chiamate a dandelion dobbiamo fare
    size_tweets = sys.getsizeof(all_tweets)
    numbOfCalls = math.ceil(size_tweets/MAX_QUERY)
    num_bytes = math.ceil(len(all_tweets)/numbOfCalls)
    print(numbOfCalls)
    ...
    for i in range(numbOfCalls):
        ...
        if (end> len(all_tweets)): #nel caso in cui la fine è dopo la fine del nostro testo
            end = len(all_tweets)
#        chiamata a dandelion
        result = callDandelion(all_tweets[start: end], datatxt)
    ...
    tweet_annotation = {}
# ora che abbiamo tutte le annotazioni che ha fatto dandelion ci dobbiamo salvare le annotazioni in mongo con l'indice di inizio e fine corretto
...
#    ordiniamo il dizionario con gli indici di fine del tweet
        keys = sorted(index_tweets.items(), key=operator.itemgetter(1))
...
#   cerchiamo l'indice del tweet in cui c'è l'annotazione ann
        ...
        id_tweet = binarySearch(keys, start)
        
        if id_tweet != 0:
            prec = keys[id_tweet-1][1]
        elif id_tweet == -1:
